...
module S3
  class Signer
    ...
    AVAILABLE_RESOURCES = %w[image proof_of_address document document_verse criminal].freeze
    ...
    def generate_unauthenticated_signature
      check_resource!
      check_extension!

      post = generate_unauthenticated_post
      SignedUploadConfig.new(post.url, post.fields)
    end
    ...
    def generate_unauthenticated_post
      signer
        .key(unauthenticated_key)
        .acl('public-read')
        .success_action_status('201')
        .expires(5.minutes.from_now)
        .content_type(type_for(@file_name))
    ...
    def unauthenticated_key
      "#{@resource_name}/#{SecureRandom.uuid}/#{mounted_file_name}"
    end
