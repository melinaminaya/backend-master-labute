...
class CuponUsageSerializer
  ...
  def serialize
    @relation.as_json(only: CuponUsageSerializer::FIELDS)
  end
  ...
  def serialize_with_pagination(page)
    ...
    {
      records: @relation.as_json(only: CuponUsageSerializer::FIELDS),
